...
export default function ProjectCard({ project, active, onOpen }: ProjectCardProps) {
  return (
    <article
      aria-hidden={!active}
      className={`mx-auto flex h-full w-full max-w-2xl flex-col overflow-hidden rounded-4xl border-2 border-ink/10 bg-white shadow-card transition-all duration-500 ${
        active
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-6 sm:p-8">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="rounded-full border-2 border-ink/10 bg-paper px-3 py-1 text-xs font-bold text-ink-soft">
            {project.tagline}
          </span>
        </div>

        <h3 className="mt-4 font-display text-3xl font-black tracking-tight text-ink sm:text-4xl">
          {project.name}
        </h3>
        ...
        <ul className="mt-4 flex flex-wrap gap-2" aria-label="Technologies">
          {project.tech.map((tech) => (
            <li
              key={tech}
              className="rounded-lg bg-accent-tint px-2.5 py-1 text-xs font-extrabold text-accent-deep"
            >
              {tech}
            </li>
          ))}
        ...
        <div className="mt-6 flex items-center justify-between gap-3">
          ...
          <span
            className="hidden items-center gap-1.5 text-[11px] font-bold text-ink-faint sm:flex"
